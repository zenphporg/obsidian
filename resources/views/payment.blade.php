...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="h-full">
  ...
  <head>
    ...
    <title>{{ __('Payment Confirmation') }} - {{ config('app.name', 'Obsidian') }}</title>
        ...
        <div class="bg-white rounded-lg shadow-xl p-4 sm:p-6 mt-4">
          ...
          <div v-else>
            ...
            <div v-if="paymentIntent.status === 'requires_payment_method'" class="mb-3">
              ...
              <p class="mb-6">
                A valid payment method is needed to process your payment. Please confirm your payment by filling out
              ...
              <div v-if="(paymentMethod || {}).remember">
                ...
                <label for="remember" class="inline-block text-sm text-gray-700 mb-2">
                  <input id="remember" type="checkbox" required class="inline-block mr-1 focus:outline-none"
                    v-model="remember" />
                ...
                <p v-if="['bancontact', 'ideal', 'sepa_debit'].includes(paymentMethod.type)"
                  ...
                  By providing your payment information and confirming this payment, you authorise (A) and Stripe, our
                  payment service provider, to send instructions to your bank to debit your account and (B) your bank to
                  debit your account in accordance with those instructions. As part of your rights, you are entitled to
                  a refund from your bank under the terms and conditions of your agreement with your bank. A refund must
                  be claimed within 8 weeks starting from the date on which your account was debited. Your rights are
                  explained in a statement that you can obtain from your bank. You agree to receive notifications for
        ...
        methods: {
          ...
          confirmPaymentMethod: function() {
            ...
            if (this.paymentMethod.redirects) {
              data.return_url = '{{ route('obsidian.payment', $paymentIntent['id']) . '?redirect=' . $redirect }}';
            }
